...
### ขั้นที่ 3: ตรวจสอบว่าได้ Python 3.10 จริง

⚠️ **ห้ามข้ามขั้นนี้** — บาง environment (เช่น conda เวอร์ชันใหม่ๆ หรือเครื่องที่เคยตั้งค่า channel ไว้) อาจไม่ได้ Python 3.10 ตามที่สั่งจริงๆ ถ้าเวอร์ชันเพี้ยนไป (เช่นได้ 3.11/3.12) ขั้นที่ 6 (ติดตั้ง YOLO) จะพังด้วย error ที่หน้าตาเหมือนปัญหา 4 ในหัวข้อแก้ปัญหาทุกประการ แต่แก้ด้วยวิธีเดิมไม่ได้ เพราะสาเหตุจริงคือ Python ผิดเวอร์ชัน ไม่ใช่ setuptools

```cmd
python --version
```

ต้องขึ้น **`Python 3.10.x`** เท่านั้น (x คือเลขอะไรก็ได้) ถ้าขึ้นเวอร์ชันอื่น ให้ลบ environment แล้วสร้างใหม่:
```cmd
conda deactivate
...
### ขั้นที่ 4: ติดตั้ง Dependency ของ Server (Flask)
...
> ℹ️ การ pin เวอร์ชันนี้ป้องกันปัญหาได้ก็ต่อเมื่อ Python ตรงกับขั้นที่ 3 จริง (3.10) เพราะแพ็กเกจในขั้นถัดไปมี wheel สำเร็จรูปสำหรับ 3.10 พอดี ไม่ต้อง build จากซอร์สเลย ถ้า Python ผิดเวอร์ชัน pip จะต้อง build จากซอร์สและสร้าง "isolated build environment" ของตัวเองซึ่งดึง setuptools เวอร์ชันล่าสุดมาเองอีกที การ pin ในขั้นนี้จะช่วยอะไรไม่ได้เลย — นี่คือเหตุผลที่ขั้นที่ 3 สำคัญมาก

### ขั้นที่ 6: ติดตั้ง Dependency ของระบบตรวจจับ (YOLO)
