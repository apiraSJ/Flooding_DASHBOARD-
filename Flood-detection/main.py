...
class FloodApp(tk.Tk):
    ...
    def _show_result(self, level, pct, title, advice):
        ...
        if level == "error":
            ...
            # อัปเดตขนาดวงน้ำท่วมอัตโนมัติตามความรุนแรง (proportional to detection area)
            sev_map = {"low": 3, "mid": 6, "high": 9}
            # Scale radius by actual flood area percentage for proportional sizing
            if level == "low":
                radius_m = 200 + int(pct * 6)   # 200-500m range
            elif level == "mid":
                radius_m = 500 + int(pct * 20)  # 500-2000m range
            else:
                radius_m = 1000 + int(pct * 40) # 1000-5000m range
            rad_map = {"low": radius_m, "mid": radius_m, "high": radius_m}
